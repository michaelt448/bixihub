...
const ComplementAccepted = (props) => {
    const classes = useStyles();

    const [complAccOpen, setcomplAccOpen] = React.useState(true);        
    const complAccClose = () => {setcomplAccOpen(!complAccOpen); props.waiting(true)};
    const complAccSubmit = () => {setcomplAccOpen(!complAccOpen); props.waiting(false); props.submit(true)};
    
    // React.useEffect(() => {
    //     setcomplAccOpen(!complAccOpen)
    //   }, [props.waiting]);
            ...
            <Box className={classes.modal}>
                ...
                <div className={classes.mb30}>
                    <Typography variant="h5" className={`${classes.typo} ${classes.mb30}`}>
                        Thank you for submitting your post! Once Scale Company has reviewed and accepted your post, you will receive your $10.
                    </Typography>
                    <Typography variant="h5" className={`${classes.typo} ${classes.mb20}`}>
                        Please rate your expeience with scale Company!
                    </Typography>
                ...
                <div className={classes.center}>
                    <Button onClick={complAccSubmit} variant="primary" className={`${classes.formButton} ${classes.formButtonXXL}`}>Submit</Button>
                </div>
            </Box>
        </Modal>
    );
